      ...
      <v-card class="mx-auto" tile>
        ...
        <v-data-table
          ...
          <template v-slot:[`item.actions`]="{ item }">
            <v-icon small class="mr-2" @click="chooseEvent(item.id)">mdi-book</v-icon>
  ...
  methods: {
    ...
    getDisplayEvent(event) {
      return {
        ...
        dept: event.dept.length > 30 ? event.dept.substr(0, 30) + "..." : event.dept,
        name: event.name.length > 30 ? event.name.substr(0, 30) + "..." : event.name,
        description: event.description.length > 30 ? event.description.substr(0, 30) + "..." : event.description,
      };
    },
  },
  mounted() {
